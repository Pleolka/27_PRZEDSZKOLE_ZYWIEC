...
export default function Grupasloneczek({ data }) {
  const dydaktyka = data.allContentfulDydaktyka.nodes

  return (
    <Container>
      <Heading>
        <h1>Grupa Słoneczek</h1>
      </Heading>
...
const DydaktykaCard = ({ item }) => {
  ...
  return (
    <>
      <Seo title="Grupa słoneczek" slug="/grupasloneczek" />
      <CardWrapper>
        <TabWrapper>
          <TabMenu>
            <CardHeader>
...
export const query = graphql`
  query SloneczkaPageQuery {
    allContentfulDydaktyka(
      filter: { grupa: { eq: "Sloneczka" } }
      sort: { data: DESC }
      limit: 12
    ) {
      nodes {
